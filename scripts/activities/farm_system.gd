...
const CROP_DATA = {
	...
	"草莓": {"days": 6, "sell_price": 25},
	"豌豆": {"days": 4, "sell_price": 15},
	"番茄": {"days": 8, "sell_price": 30},
	"黄瓜": {"days": 6, "sell_price": 20},
	"西瓜": {"days": 10, "sell_price": 50},
	"茄子": {"days": 7, "sell_price": 25},
	"南瓜": {"days": 9, "sell_price": 35},
	"玉米": {"days": 8, "sell_price": 28},
	"苹果": {"days": 12, "sell_price": 40},
	"葡萄": {"days": 10, "sell_price": 38},
	"菠菜": {"days": 5, "sell_price": 18},
	"胡萝卜": {"days": 6, "sell_price": 22}
}
...
func _ready() -> void:
	...
	init_plots()
	print("[Farm] 后院田地系统初始化完成，共 ", TOTAL_PLOTS, " 块田地")
...
func plant(plot_index: int, crop_type: String) -> bool:
	...
	if plot.state != 0:
		print("[Farm] 田地 ", plot_index, " 已有作物，无法种植")
		return false
...
func harvest(plot_index: int) -> bool:
	...
	var plot = plots[plot_index]
	if plot.state != 3:
		print("[Farm] 田地 ", plot_index, " 作物还未成熟")
		return false
