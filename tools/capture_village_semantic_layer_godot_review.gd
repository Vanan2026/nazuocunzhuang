...
const DEFAULT_OUTPUT_DIR := "res://.codex/village_semantic_layer_godot_review"
const LAYER_MANIFEST_PATH := "res://production/assets/regions/village_world2d/v001/03_layer_export/layer_export_manifest.json"
const WATCHDOG_TIMEOUT_SECONDS := 45.0
const VIEWPORT_SIZE := Vector2i(1280, 720)
const SEMANTIC_LAYER_SCALE := Vector2(0.18, 0.18)

const OUTPUT_FILENAMES := {
	"overview": "01_village_semantic_overview.png",
	"old_maple_occlusion_focus": "02_old_maple_occlusion_focus.png",
	"seed_stall_notice_focus": "03_seed_stall_notice_focus.png",
	"east_continuity_focus": "04_village_east_continuity_focus.png",
}
...
func _run() -> void:
	print("PROGRESS: Village semantic layer Godot review initialize")
	if not _check_only and DisplayServer.get_name() == "headless":
		_fail("Village semantic layer Godot review capture requires a display server; rerun with --check-only for headless validation")
		return
	if not _check_only:
	...
	if manifest.is_empty():
		return
	if not _install_semantic_layers(village_section, manifest):
		return
	...
	if _check_only:
		print("OK: Village semantic layer Godot review check-only validated")
	else:
		print("OK: Village semantic layer Godot review screenshots saved to %s" % ProjectSettings.globalize_path(_output_dir))
	_finish_deferred(0)
...
func _load_layer_manifest() -> Dictionary:
	...
	if text.is_empty():
		_fail("missing semantic layer manifest: %s" % LAYER_MANIFEST_PATH)
		return {}
	var parsed: Variant = JSON.parse_string(text)
	if not (parsed is Dictionary):
		_fail("semantic layer manifest is not a JSON object")
		return {}
	var manifest: Dictionary = parsed
	if not _expect(manifest.get("status") == "semantic_rework_exported_pending_review", "manifest must be semantic_rework_exported_pending_review"):
		return {}
...
func _install_semantic_layers(village_section: Node2D, manifest: Dictionary) -> bool:
	# review-only integration: installed at runtime for screenshot evidence only.
	_hide_existing_generated_layers(village_section)
	_hide_authored_blockout_visuals(village_section)
	var existing := village_section.get_node_or_null("VillageSemanticLayerReview")
	if existing != null:
		existing.queue_free()
	var root_node := Node2D.new()
	root_node.name = "VillageSemanticLayerReview"
	root_node.z_as_relative = false
	village_section.add_child(root_node)

	var layers: Array = manifest.get("layers", [])
	if not _expect(layers.size() == 5, "semantic review expects five layer records"):
		return false
	for layer_record in layers:
		if not (layer_record is Dictionary):
			_fail("semantic layer record must be object")
			return false
...
func _validate_review_runtime_state(outdoor: Node, village_section: Node2D) -> bool:
	var review_root := village_section.get_node_or_null("VillageSemanticLayerReview")
	if not _expect(review_root != null, "semantic review root should be installed"):
		return false
	...
	if not _expect(base != null and base.texture != null, "semantic review needs BaseGround texture"):
		return false
	if not _expect(foreground != null and foreground.texture != null, "semantic review needs ForegroundOcclusion texture"):
		return false
	...
	if not _expect(notice != null and stall != null and old_maple != null, "Village interactables must remain present under review art"):
		return false
	var active_id := String(outdoor.call("get_active_region_id"))
	return _expect(active_id == "village", "OutdoorWorld must remain in Village while reviewing semantic layers")
...
func _capture_overview(outdoor: Node) -> bool:
	...
	if not _expect(camera != null, "OutdoorWorld should expose WorldCamera for semantic review"):
		return false
	camera.enabled = true
	camera.make_current()
...
func _capture_notice_stall_focus(outdoor: Node, village_section: Node2D) -> bool:
	var player := _get_player(outdoor)
	if not _expect(player != null, "semantic review should have Player for notice focus"):
		return false
	...
	await _capture_review_frame("seed_stall_notice_focus")
	return not _has_failed
...
func _capture_east_continuity_focus(outdoor: Node, village_section: Node2D) -> bool:
	...
	if not _expect(mountain_hut_section != null, "OutdoorWorld should contain MountainHut section for east continuity review"):
		return false
	var player := _get_player(outdoor)
	if not _expect(player != null, "semantic review should have Player for east continuity focus"):
		return false
...
func _capture_review_frame(key: String) -> void:
	...
	await process_frame
	if _check_only:
		print("OK: checked Village semantic layer frame: %s" % key)
		return
	...
	if error != OK:
		_fail("could not save Village semantic layer screenshot %s: %s" % [output_path, error])
		return
	print("OK: captured Village semantic layer frame: %s" % output_path)
...
func _start_watchdog() -> void:
	...
	_watchdog_timer.timeout.connect(func() -> void:
		if not _finished:
			_fail("Village semantic layer Godot review timed out")
	)
